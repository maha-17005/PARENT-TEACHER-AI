            ...
            <div class="table-responsive bg-white">
              <table class="table table-bordered">
                ...
                <tbody>
                      ...
                      $stmt = $db_conn->prepare("SELECT p.*, m.* FROM `posts` as p INNER JOIN `metadata` as m ON p.id = m.item_id WHERE p.`type` = 'study-material' AND m.meta_key = 'class' AND m.meta_value = ?");
                      $stmt->bind_param("s", $class);
                      $stmt->execute();
                      $result = $stmt->get_result();

                      while ($att = $result->fetch_object()) {
